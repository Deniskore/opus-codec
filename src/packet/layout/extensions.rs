...
#[cfg(not(opus_codec_frame_bounded_extensions))]
fn parse_extensions(data: &[u8], _frame_count: usize) -> Result<Vec<PacketExtension<'_>>> {
...
impl<'a> ExtensionIteratorFrameBounded<'a> {
    ...
    fn next_extension(&mut self) -> Result<Option<PacketExtension<'a>>> {
            ...
            while self.curr_pos < self.data.len() {
                ...
                if id == EXTENSION_FRAME_SEPARATOR_ID {
                    ...
                    return Ok(Some(PacketExtension {
                        ...
                        data: &self.data[data_start..next],
                    }));
                }
            }
            // A trailing repeat marker is valid when all repeated short
            // extensions have empty payloads; process it before treating EOF
            // as the end of the extension stream.
    ...
    fn next_repeated_extension(&mut self) -> Result<Option<PacketExtension<'a>>> {
            ...
            while self.repeat_frame < self.nb_frames {
                while self.repeat_pos < self.repeat_end {
                    ...
                    return Ok(Some(PacketExtension {
                        ...
                        data: &self.data[data_start..payload_next],
                    }));
                }
                self.repeat_pos = self.repeat_data;
                self.repeat_frame += 1;
...
#[cfg(opus_codec_frame_bounded_extensions)]
fn parse_extensions(data: &[u8], frame_count: usize) -> Result<Vec<PacketExtension<'_>>> {
...
fn extension_output_capacity_hint(extensions: &[PacketExtension<'_>], len_limit: usize) -> usize {
    extensions
        .iter()
        .fold(0usize, |capacity, extension| {
            capacity.saturating_add(
                extension
                    .data
                    .len()
                    .saturating_add(extension.data.len() / EXTENSION_LENGTH_CHUNK)
                    .saturating_add(4),
            )
        })
        .min(len_limit)
...
pub(super) fn generate_extensions(
    ...
    let mut out = Vec::with_capacity(extension_output_capacity_hint(extensions, len_limit));
    let mut current_frame = 0usize;
...
#[cfg(opus_codec_frame_bounded_extensions)]
struct FrameBoundedExtensionWriter<'extensions, 'data> {
    extensions: &'extensions [PacketExtension<'data>],
    len_limit: usize,
    extension_count: usize,
    frame_count: usize,
    frame_min_idx: [usize; MAX_FRAMES_PER_PACKET],
...
#[cfg(opus_codec_frame_bounded_extensions)]
impl<'extensions, 'data> FrameBoundedExtensionWriter<'extensions, 'data> {
    fn new(
        extensions: &'extensions [PacketExtension<'data>],
        len_limit: usize,
        frame_count: usize,
        ...
        let mut frame_min_idx = [extension_count; MAX_FRAMES_PER_PACKET];
        ...
        let frame_repeat_idx = frame_min_idx;
        Ok(Self {
            extensions,
            len_limit,
            extension_count,
            ...
            frame_max_idx,
            frame_repeat_idx,
            out: Vec::with_capacity(extension_output_capacity_hint(extensions, len_limit)),
            current_frame: 0,
            written: 0,
        })
    }
