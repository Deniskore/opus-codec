...
#[test]
fn packet_pad_matches_opus_for_trailing_repeat_with_empty_payloads() {
    let payload = [0x7B, 0x45, 2, b'a', b'b', b'c', b'd', b'e', 0x12, 0x05];
    let mut packet = vec![0u8; 32];
    packet[..payload.len()].copy_from_slice(&payload);
    let new_len = packet.len();
    assert_packet_pad_matches_opus(&mut packet, payload.len(), new_len);
}
...
#[test]
fn multistream_pad_matches_opus_when_streams_exhaust_packet() {
    let mut packet = [0x00, 0x00, 0x00];
    let mut c_packet = packet;
    let c_result =
        c_pad_result(unsafe { opus_multistream_packet_pad(c_packet.as_mut_ptr(), 2, 3, 2) });

    assert_eq!(c_result, Err(Error::BadArg));
    assert_eq!(multistream_packet_pad(&mut packet, 2, 3, 2), c_result);
}
