...
#[cfg(opus_codec_frame_bounded_extensions)]
fn five_frame_packet_with_repeated_empty_extension() -> [u8; 10] {
    [
        TOC_CONFIG | PACKET_CODE_3,
        CODE_3_PADDING_FLAG | 5,
        2,
        b'a',
        b'b',
        b'c',
...
#[test]
fn test_repacketizer_accepts_owned_packet_without_changing_output() {
    let packet = single_frame_packet(b'x').to_vec();
...
#[cfg(opus_codec_frame_bounded_extensions)]
#[test]
fn test_repacketizer_emit_handles_trailing_repeat_with_empty_payloads() {
    let packet = five_frame_packet_with_repeated_empty_extension();
    let mut rp = Repacketizer::new().unwrap();
    rp.push(&packet).unwrap();
    ...
    let out_len = rp.emit_range(1, 5, &mut out).unwrap();
    assert_eq!(
        &out[..out_len],
        &[
            TOC_CONFIG | PACKET_CODE_3,
            CODE_3_PADDING_FLAG | 4,
            2,
            b'b',
            b'c',
            b'd',
