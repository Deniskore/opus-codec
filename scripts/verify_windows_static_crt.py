...
def target_arch() -> str:
    arch = TARGET_ARCHES.get(TARGET)
    if not arch:
        ci_utils.fail(f"Unsupported MSVC verification target: {TARGET}")
    return arch
...
def find_dumpbin() -> Path:
    ...
    candidates = sorted(
        Path(install_path).glob(
            f"VC/Tools/MSVC/*/bin/Hostx64/{target_arch()}/dumpbin.exe"
        )
    )
    if not candidates:
...
def build_test_executables() -> List[Path]:
    ...
    build_script = ci_utils.cargo_root_build_script(result.stdout)
    out_dir = Path(build_script["out_dir"])
    cache = out_dir / "build" / "CMakeCache.txt"
    if not cache.is_file():
        ci_utils.fail(f"Bundled Opus CMake cache not found at {cache}")
    if not cache_bool(cache, "OPUS_STATIC_RUNTIME"):
        ci_utils.fail(f"Bundled Opus was not configured with a static CRT in {cache}")
    print(f"Verified OPUS_STATIC_RUNTIME=ON in {cache}")

    target_root = (TARGET_DIR / TARGET).resolve()
    executables: List[Path] = []
    for message in ci_utils.cargo_json_messages(result.stdout):
...
def imported_dlls(dumpbin: Path, executable: Path) -> List[str]:
    output = ci_utils.run(
        [str(dumpbin), "/nologo", "/dependents", str(executable)],
        capture_output=True,
    ).stdout
    imports = []
    ...
    if not imports:
        ci_utils.fail(
            f"dumpbin reported no recognizable DLL imports for {executable}; "
            "refusing to treat an empty parse as success"
        )
    return imports
